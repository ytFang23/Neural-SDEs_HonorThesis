...
parser.add_argument("--plot-final", action="store_true", help="After training, load the best checkpoint and plot final reconstructions.")
...
if __name__ == "__main__":
	...
	if args.rnn_vae:
		...
		z0_diffeq_solver = SDESolver(input_dim, rec_ode_func, "euler", args.latents,
		...
		# Build SDE func + solver
		from lib.de_func import NeuralSDEFunc
		from lib.diffeq_solver import SDESolver

		sde_func = NeuralSDEFunc(
			input_dim=input_dim,  # for API symmetry
			hidden_dim=args.latents,  # latent dim
			hidden_hidden_dim=args.units,  # width of MLP inside f/g
			num_layers=args.gen_layers  # depth of MLP inside f/g
		).to(device)
		...
		# Create SDE-RNN model (接口与 ODE-RNN/Classic RNN 对齐)
		model = SDE_RNN(
			input_dim=input_dim,
			...
			n_units=args.units,
			obsrv_std=obsrv_std,  # 与 run_models 里其它 baseline 一致
			use_binary_classif=args.classif,  # PhysioNet/Activity 可用
			classif_per_tp=("classif_per_tp" in data_obj and data_obj["classif_per_tp"]),
			linear_classifier=args.linear_classif,
			n_labels=(data_obj["n_labels"] if ("n_labels" in data_obj) else 1),
			train_classif_w_reconstr=(args.dataset == "physionet")
		).to(device)
	...
	for itr in range(1, num_batches * (args.niters + 1)):
		...
		if itr % (n_iters_to_viz * num_batches) == 0:
			with torch.no_grad():
				...
				if select_metric in test_res:
					val_value = float(test_res[select_metric])
				else:
					# fallback if the key is missing
					val_value = float(test_res.get("mse", 1e9))

				val_value = float(test_res.get(select_metric, test_res.get("mse", test_res["loss"])))
