...
##################################################################
    ...
    metrics_hist = {"accuracy": [], "auc": [], "mse": []}  # running history across evals
    metrics_path: Path = _LOGS_DIR / f"metrics_{file_name}_{experimentID}.jsonl"
...
for itr in range(1, num_batches * (args.niters + 1)):
        ...
        if itr % (n_iters_to_viz * num_batches) == 0:
            ...
            for k in ("accuracy", "auc", "mse"):
                if k in test_res:
                    try:
                        metrics_hist[k].append(float(test_res[k]))
                    except Exception:
                        pass


            # Compute running stats and print to terminal every eval
            def _stats(arr):
                t = torch.tensor(arr, device=device, dtype=torch.float32)
                return t.mean().item(), t.var(unbiased=False).item(), t.std(unbiased=False).item()


            for k in ("accuracy", "auc", "mse"):
                vals = metrics_hist.get(k, [])
                if len(vals) > 0:
                    mean_k, var_k, std_k = _stats(vals)
                    logger.info(
                        f"Running TEST {k} mean/std/var over {len(vals)} evals: "
                        f"{mean_k:.6f} / {std_k:.6f} / {var_k:.6f}"
                    )

            # Save metrics every 10 evaluations (epochs)
            # Eval happens every n_iters_to_viz * num_batches updates; we use epoch index.
            epoch_idx = itr // num_batches
            if (epoch_idx % 10) == 0:
                payload = {
                    "time": datetime.datetime.now().isoformat(timespec="seconds"),
                    "epoch": int(epoch_idx),
                    "iter": int(itr),
                    "metrics_hist": {k: metrics_hist[k] for k in metrics_hist},  # raw history so far
                }
                try:
                    ...
                    logger.info(f"[metrics] saved running metrics to {metrics_path}")
                except Exception as e:
                    logger.info(f"[metrics] failed to save metrics: {e}")
