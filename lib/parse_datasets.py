...
def parse_datasets(args, device):
	def basic_collate_fn(batch, time_steps, args=args, device=device, data_type="train"):
		batch = torch.stack(batch).to(device)  # (B,T,D)
		mask = torch.ones_like(batch)  # SDE-RNN 需要
		data_dict = {"data": batch, "time_steps": time_steps.to(device).float(), "mask": mask}
	...
	if dataset_name == "periodic" and getattr(args, "use_sde_noise", False):
		# Use SDE-driven noise (OU / Brownian) from your updated Periodic_1d
		# Periodic_1d.sample_traj accepts numpy or torch; we pass numpy for safety
		ts_np = time_steps_extrap.cpu().numpy()
		dataset = dataset_obj.sample_traj(
			ts_np,
			n_samples=args.n,
			sde_noise_type=args.sde_noise_type,  # "ou" or "bm"
			sde_noise_sigma=args.sde_sigma,
			sde_ou_theta=args.ou_theta,
			sde_ou_mu=args.ou_mu
		)
	else:
		# Fallback to legacy noise mechanism (your original behavior)
		dataset = dataset_obj.sample_traj(
			time_steps_extrap.cpu().numpy(),  # or time_steps_extrap if you prefer torch
			n_samples=args.n,
			# legacy path ignores SDE params; you can keep old "noise_weight" arg if present
			# If your new Periodic_1d no longer supports "noise_weight", just pass very small BM:
			sde_noise_type="bm",
			sde_noise_sigma=float(getattr(args, "noise_weight", 0.0))  # keep old CLI compatible, else 0.0
		)
