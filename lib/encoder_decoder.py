...
class Encoder_z0_SDE_RNN(nn.Module):
    # Derive z0 by running sde backwards.
    # For every y_i we have two versions: encsded from data and derived from SDE by running it backwards from t_i+1 to t_i
    # Compute a weighted sum of y_i from data and y_i from sde. Use weighted y_i as an initial value for SDE runing from t_i to t_i-1
    ...
    def __init__(self, latent_dim, input_dim, z0_diffeq_solver=None,
        ...
        super(Encoder_z0_SDE_RNN, self).__init__()
    ...
    def forward(self, data, time_steps, run_backwards=True, save_info=False):
        ...
        if len(time_steps) == 1:
            ...
            last_yi, last_yi_std, _, extra_info = self.run_sdernn(
    ...
    def run_sdernn(self, data, time_steps,
        ...
        latent_ys = []
        # Run SDE backwards and combine the y(t) estimates using gating
        ...
        for i in time_points_iter:
            if (prev_t - t_i) < minimum_step:
                ...
                inc = self.z0_diffeq_solver.sde_func(prev_t, prev_y) * (t_i - prev_t)

                assert (not torch.isnan(inc).any())

                sde_sol = prev_y + inc
                sde_sol = torch.stack((prev_y, sde_sol), 2).to(device)

                assert (not torch.isnan(sde_sol).any())
            else:
                n_intermediate_tp = max(2, ((prev_t - t_i) / minimum_step).int())

                time_points = utils.linspace_vector(prev_t, t_i, n_intermediate_tp)
                sde_sol = self.z0_diffeq_solver(prev_y, time_points)

                assert (not torch.isnan(sde_sol).any())

            err = (sde_sol[:, :, 0, :] - prev_y).abs().max().item()
            if err > 5e-2:
                print(f"[warn] first SDE point != init, |Δ|≈{err:.3e} (dt may be coarse / times nudged)")
            # no exit()

            # assert(torch.mean(sde_sol[:, :, 0, :]  - prev_y) < 0.001)

            yi_sde = sde_sol[:, :, -1, :]
            xi = data[:, i, :].unsqueeze(0)

            yi, yi_std = self.GRU_update(yi_sde, prev_std, xi)
            ...
            if save_info:
                d = {"yi_sde": yi_sde.detach(),  # "yi_from_data": yi_from_data,
                     "yi": yi.detach(), "yi_std": yi_std.detach(),
                     "time_points": time_points.detach(), "sde_sol": sde_sol.detach()}
                extra_info.append(d)
